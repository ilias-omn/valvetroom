...
export default function LiveRoom() {
  const { id: performerId } = useParams<{ id: string }>();
  ...
  // The shared call/room id for this live session
  const roomId = `live-${performerId}`;
  ...
  useEffect(() => {
    ...
    s.on('connect', () => {
      s.emit('user-identify', { userId: user.id, username: user.username, role: user.role, page: `/live/${performerId}` });
      s.emit('call-join', {
        callId: roomId,
        role: user.role,
  ...
  const endSession = (fromRemote = false) => {
    ...
    if (!fromRemote && socketRef.current) {
      socketRef.current.emit('end-call', { callId: roomId });
    }
      ...
      <div className="bg-dark-800 border-b border-dark-600 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className={`flex items-center gap-2 text-sm ${status === 'active' ? 'text-green-400' : 'text-yellow-400'}`}>
            ...
            {status === 'active' ? 'Live' : 'Waiting for other party...'}
          </div>
          {status === 'active' && (
